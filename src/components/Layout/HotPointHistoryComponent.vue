        ...
        <div v-if="historyDataBoardShow" class="history-backdrop" @click="loading ? null : close">
          <div class="history-panel" @click.stop>
                ...
                <div class="title-info" style="display: flex; flex-direction: column; gap: 12px; width: 100%;">
                  <div style="display: flex; align-items: center; gap: 10px; width: 100%;">
                    ...
                    <button
                        ...
                        :disabled="loading"
                        :style="{ background: loading ? '#2c3e50' : '#409eff', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }"
                        style="border:none; height:36px; padding:0 20px; font-size: 14px;"
                    >
                      {{ loading ? '分析中...' : '分析' }}
                    </button>
                  </div>
                  <div v-if="showHistoryGrid && searchQuery.length === 0 && searchHistory.length > 0" style="display: flex; flex-wrap: wrap; gap: 8px; padding-top: 10px; border-top: 1px solid rgba(255,255,255,0.05);">
                    <div v-for="(item, index) in searchHistory" :key="index" @click="selectHistory(item)" style="background: rgba(255,255,255,0.05); padding: 4px 10px; border-radius: 6px; font-size: 12px; color: #8892b0; cursor: pointer;">{{ item }}</div>
                  </div>
                </div>
              </div>
              ...
              <div class="chart-tabs">
                <div class="tab-buttons">
                  <button
                      ...
                      :style="{ cursor: loading ? 'not-allowed' : 'pointer', opacity: loading && currentSearchMode !== tab.mode ? 0.5 : 1 }"
                  >
                    {{ tab.label }}
                  </button>
                </div>
                  ...
                  <div class="chart-container">
                    ...
                    <div class="chart-wrapper">
                      ...
                      <div class="chart-placeholder" @mousemove="onChartHover($event)" @mouseleave="hideTooltip">
                        <div v-if="loading" style="position:absolute; inset:0; display:flex; align-items:center; justify-content:center; z-index:10;">
                          <div class="atom-spinner"><div class="spinner-inner"><div class="spinner-line"></div><div class="spinner-line"></div><div class="spinner-line"></div><div class="spinner-circle">&#9679;</div></div></div>
                        </div>
                        ...
                        <div class="chart-x-labels" :style="{ opacity: loading ? 0.3 : 1 }">
              ...
              <div class="history-list">
                ...
                <div class="list-items">
                  <div v-if="loading" style="display:flex; flex-direction:column; align-items:center; padding:40px;">
                    <div class="atom-spinner">
                      <div class="spinner-inner">
                        <div class="spinner-line"></div><div class="spinner-line"></div><div class="spinner-line"></div>
                  ...
                  <div v-else>
                    ...
                    <div v-if="historyData.length === 0" style="text-align:center; padding:40px; color:#6b7280;">暂无历史数据</div>
                  </div>
                </div>
              </div>
            </main>
            <footer class="panel-footer">
              ...
              <button class="close-btn-footer" @click="close" :disabled="loading" :style="{ opacity: loading ? 0.5 : 1 }">关闭追踪</button>
            </footer>
          </div>
        </div>
      </transition>
...
export default {
  ...
  computed: {
    historyDataBoardShow() { return this.$store.state.historyDataBoardShow; },
    isHistoryMode() { return this.tabConfigs.find(t => t.mode === this.currentSearchMode)?.isHistory || false; },
    ...
    dataCounts() {
      if (this.isHistoryMode) {
        const map = new Map();
        this.historyData.forEach(item => { const d = item.dataUpdateTime.slice(0, 10); map.set(d, (map.get(d) || 0) + 1); });
  ...
  methods: {
    ...
    hideTooltip() { this.tooltipShow = false; },
    loadLocalHistory() { const h = getLocalStorage(LOCAL_STORAGE_KEYS.SEARCH_HISTORY); if (h) this.searchHistory = JSON.parse(JSON.stringify(h)); },
    saveLocalHistory(k) {
      const h = [k.trim(), ...this.searchHistory.filter(i => i !== k.trim())].slice(0, 15);
      this.searchHistory = h; setLocalStorage(LOCAL_STORAGE_KEYS.SEARCH_HISTORY, JSON.stringify(h));
    },
    selectHistory(k) {
      if (this.loading) return;
      this.searchQuery = k;
    ...
    close() { if (this.loading) return; this.$store.commit('setHistoryDataBoardShow', false); },
    handleKey(e) { if (e.key === 'Escape' && this.historyDataBoardShow && !this.loading) this.close(); }
  }
};
</script>
...
.history-panel {
  position: relative; width: 80%; max-width: 90%; height: 90vh;
...
.close-btn:hover:not(:disabled) { background: rgba(255, 255, 255, 0.1); transform: scale(1.05); }
...
.hot-title-card { background: rgba(40, 44, 56, 0.5); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 18px; padding: 12px; margin-bottom: 28px; }
...
.chart-placeholder { position: relative; flex: 1; height: 180px; background: rgba(0, 0, 0, 0.2); border-radius: 12px; cursor: crosshair; }
.fake-line-chart { position: absolute; inset: 0; padding: 16px 16px 30px 16px; transition: opacity 0.3s; }
...
.chart-tooltip { position: absolute; background: rgba(20, 23, 32, 0.98); backdrop-filter: blur(16px); border: 1px solid rgba(64, 158, 255, 0.3); border-radius: 12px; padding: 12px 14px; box-shadow: 0 8px 32px rgba(0, 0, 0, 0.8); min-width: 280px; max-width: 360px; z-index: 1000; font-size: 13px; }
...
.tooltip-header strong { font-size: 13px; color: #e6edf3; }
.tooltip-header .count { font-size: 11px; color: #67c23a; background: rgba(103, 194, 58, 0.15); padding: 2px 8px; border-radius: 10px; }
.tooltip-list { display: flex; flex-direction: column; gap: 8px; max-height: 140px; overflow-y: auto; }
.tooltip-item { display: flex; gap: 8px; font-size: 12px; }
...
.relevance-value { font-size: 13px; color: #67c23a; font-weight: 600; }
...
.relevance-bar { height: 100%; background: linear-gradient(90deg, #409eff, #67c23a); }
.panel-footer { padding: 20px 32px 32px; display: flex; justify-content: space-between; align-items: center; }
.shortcuts kbd { background: rgba(255, 255, 255, 0.08); padding: 4px 8px; border-radius: 6px; font-size: 11px; color: #6b7280; border: 1px solid rgba(255, 255, 255, 0.1); }
.close-btn-footer { padding: 12px 24px; background: rgba(255, 255, 255, 0.08); color: #e6edf3; border-radius: 12px; cursor: pointer; transition: all 0.3s; }

.atom-spinner { height: 50px; width: 50px; }
.atom-spinner .spinner-inner { position: relative; height: 100%; width: 100%; }
.atom-spinner .spinner-circle { position: absolute; color: #ff1d5e; top: 90%; left: 90%; transform: translate(-90%, -90%); }
.atom-spinner .spinner-line { position: absolute; width: 100%; height: 100%; border-radius: 90%; border-left: 4px solid #ff1d5e; border-top: 2px solid transparent; }
